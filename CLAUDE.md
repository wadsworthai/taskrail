...
CHANGELOG.md           # generated by relscribe at release; never edited on a task branch
relscribe.toml         # how relscribe versions and tags a release (README: Releasing)
...
├── pr-title.yml       # `relscribe lint` on the pull request title
...
title becomes the only commit on `main`, which relscribe reads to compute the next version and
write the changelog (README: *Releasing*). CI lints the title with `relscribe lint`.
