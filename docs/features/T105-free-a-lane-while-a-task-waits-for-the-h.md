...
**2. The human's answer requeues the task.** A new lane state `queued`, recorded with

```bash
taskrail autopilot lane <ID> --run <R> --state queued [--reason "<what was answered>"]
```
...
or `queued` — a task with no lane to resume cannot jump the queue.

**3. `next` redispatches queued tasks first.** Before the ordinary candidates, `next --run R` takes
the run's `queued` tasks, oldest answer first (`updated`, then ID), subject to `max_lanes`, group
...
**4. When every lane is busy, the answer waits.** A queued task that no free lane takes stays
`queued` and is reported, so the orchestrator never loses it:

- `next` gains a `queued` list — `[{id, run, gate, reason, dispatched, why}]` — holding every queued
...
**5. The skill answers escalations through the queue.** `taskrail-autopilot`'s *Escalate* step
gains: when the human answers, record `--state queued`, then run `next --run <R>`; resume or
...
4. `autopilot lane <ID> --run R --state queued` records `queued`, keeps the recorded `gate` and
   accepts an optional `--reason`; `status` and `next` read the task as `queued`.
5. `--state queued` exits 5 for a task whose state is `pending`, `dispatched`, `done-branch`,
   `handed-off`, `done-merged`, `discarded` or `discarded-branch`, naming the states it allows.
6. `next --run R` dispatches a queued task before a never-started candidate, with `restart: true`
...
7. A queued task is redispatched even when the run's `remaining` is 0, and a redispatch does not
   lower `remaining`.
8. Two queued tasks are redispatched oldest answer first.
9. With every lane busy, `next --run R` dispatches no queued task, reports each in `queued` with
   `dispatched: false` and `why: "no free lane"`, and reports `limited_by: "max_lanes"`; the text
   form names them.
10. `next` without `--run` (the preview) lists open runs' queued tasks in `queued` with
    `why: "needs --run <R>"` and dispatches none, writing nothing.
11. `status` reports `holds_lane` for every task row: false for `escalated`, `queued`, `failed`,
...
12. A queued task's escalation flags are unchanged by the new state: an unapproved governing path
...
| File | Change |
|---|---|
| `src/taskrail/autopilot/dispatch.py` | `OCCUPYING` drops `escalated`; `COUNTED` gains `queued`; a queued pass before the candidate loop; `queued` in the report; `text()` lines |
| `src/taskrail/autopilot/runs.py` | `LANE_STATES` gains `queued`; `GATE_CLEARING` unchanged (`queued` keeps its gate); a helper for the queued lanes of a run |
| `src/taskrail/autopilot/status.py` | `STATES` and `WITH_BRANCH` gain `queued`; `RECORDED` gains `queued`; `holds_lane` per row |
| `src/taskrail/autopilot/commands.py` | `--state queued` in the parser, its refusal, `holds_lane` in `_status_text`, the `queued` lines in `next`'s text |
...
| `tests/` | new `tests/test_autopilot_queue.py`; edits to `tests/test_autopilot_next.py::test_gate_and_escalated_occupy_a_lane_and_failed_does_not` (its name and premise change) and to the state assertions in `tests/test_autopilot.py` |
...
- **`failed` → `queued` by a command other than `lane`**, and any new top-level command.

## Open questions and risks

Each is a decision for the plan gate; recommendations are in the *Behaviour* section.

1. **The state's name.** `queued` is the row's own word ("returns to the queue"), but `status`
   already has `handoff.queue` for branches waiting to be handed off. Alternatives: `ready`,
   `waiting`, `parked`.
2. **Priority against tasks never started.** Recommended: queued tasks always first. Alternative:
   merge them into the backlog order, which can leave an answered task behind a new one.
3. **Resources.** Recommended: an escalated or queued lane releases its values, as a failed one
   does. Alternative: hold them, which turns the stall into `limited_by: resource:<name>` when
   there are as many values as lanes.
4. **What `next` records on redispatch.** Recommended: the lane returns to `running` with a live
   `dispatched` stamp. Alternative: keep `queued` recorded and treat a live dispatch as occupying,
   which self-heals after `claim_grace_minutes` if the orchestrator never restarts the lane, at the
   cost of a conditional in three places.
5. **Always through the queue.** Recommended: an answered escalation is always recorded `queued`
   and returns through `next`, even when a lane is free. Alternative: let the orchestrator resume
   directly when `status` shows a free lane, which is one step shorter and one more thing to get
   wrong.
6. **Risk: work in flight outgrows the lanes.** With lanes freed, a run can hold `max_lanes`
   working lanes plus any number of escalated workspaces. That is what the row asks for, and
   `status` now makes it visible through `holds_lane`.
