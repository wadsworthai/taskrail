...
Status: **released as `v0.4.0`.** See CHANGELOG.md.
...
```toml
version = "v0.4.0"          # CLI version the repository is pinned to
...
```bash
uvx --from "git+https://github.com/wadsworthai/taskrail.git@v0.4.0" taskrail init --integration claude
...
repository, and overrides that default. `TASKRAIL_BIN` overrides the wrapper
...
| `autopilot next [--run R]` | *Implemented (T030).* Exit 5, naming `[autopilot].enabled`, unless it is true — checked first, preview included; then exit 5 when a kind the run drives — a named run's named tasks' kinds — or, for the preview, `[autopilot].kinds` commits on done (§12.2; T081); then exit 1 for an invalid backlog, 3 for an unknown run, 4 when the lock cannot be taken, and 0 otherwise, even when nothing is dispatched. Tasks to dispatch now: `taskrail next`'s eligible tasks in its order (so a single unmerged dependency is a stacked base, and claimed or blocked tasks are not candidates) — on a named run only its `named` tasks, in the order named, a named task that is not eligible and holds no place in the run being listed in `skipped` with why it waits (`blocked by …`, `claimed by <owner>`, `claimed in run …`, `done-branch`, …; T071) — of the kinds the run drives — the run's `kinds` and `[autopilot].kinds` intersected when both are set, whichever is set otherwise, every allowed kind when neither is. A candidate is skipped, with its reason, when it is `done-merged` or `discarded` (§12.4) while its row in the checkout is still `⬜`, as after a merge that was not pulled (*T064*), recorded `failed` in the run, dispatched or otherwise occupying a lane (`running`, `gate`, `escalated`) in any run — such as a lane between `done` and its commit, in a group at its limit, or when its base is diverged or missing; it also skips a task whose `base.row` is `missing`, with the reason (T070), and, on a named run, a named task of a kind the run does not drive (T071). Before them, on `next --run R`, the run's `parked` tasks — escalations the human has answered — are sent back, oldest answer first, subject to `max_lanes`, the group limits and the resources but not to the run's count, which already holds them (§12.7; T105). Candidates are taken in order while a lane is free (`max_lanes`), the run's count allows more (§12.7) and every resource has a free value; `limited_by` names the limit that stopped it. For each task: `show --json`'s fields (its `base` already carries `commit`, and `prior_work.prepared` says whether its branch is a prepared workspace, §7.2), the allocated `resources`, `environment` (`TASKRAIL_RESOURCE_<NAME>`), `groups`, `restart` (true for a parked task returning to its own branch and worktree; T105), and the decision-record paths; also `lanes`, `remaining`, `groups`, `resources`, `released`, `skipped` and `parked` — every parked task of every open run, with its `run`, `gate`, `reason`, whether this call `dispatched` it and, when it did not, `why`: `no free lane`, `group <name> is full`, `resource:<NAME>`, or `needs --run <R>` in the preview (T105). Under the common-directory lock, in one step, it releases the resources of lanes that ended and records `dispatched` and `resources` for each dispatched task in run R, a parked task returning to `running` with its `gate` and `reason` cleared (T105). Without `--run` it is a preview for `[autopilot].kinds` with no count, and writes nothing. Claiming stays the lane's job. |
| `autopilot lane <ID> --run R [--handle H] [--group G] [--state running\|gate\|escalated\|parked\|failed\|handed-off] [--reason …] [--gate STAGE]` | *Implemented (T029; `--gate` T032; `close` T050).* Records the agent-specific lane handle, a group membership assigned by judgement (§12.7; exit 2 unless `[[autopilot.group]]` has a group of that name without `column` and `match`) and the orchestrator's view of the lane. `--reason` is required with `escalated` and `failed`, optional with `gate` and `parked`, and cleared by `running`. `escalated`, `parked` and `failed` keep the claim, so the task stays ineligible and its dependents stay blocked, and each frees its lane (§12.7). `parked` records that the human has answered an escalation and the task waits for a lane: it keeps the recorded `gate`, and `next --run R` sends it back before any task that has never started (T105). It exits 5 unless the task is `running`, `gate`, `escalated`, `failed` or `parked` — a task that holds no lane cannot wait for one. `handed-off` appends the task to the run's hand-off order once, and exits 5 unless the task is `done-branch` or `discarded-branch` (T065). `--gate` records the stage whose gate the lane is stopped at, as `gate`: it must be a stage of the task's kind, or `close` for the stop after `taskrail done` that every kind shares, and the lane must be (or be set) `gate` or `escalated`, otherwise exit 2; a move to `gate` or `escalated` keeps it, `running` and `failed` clear it. An unknown run or task exits 3. |
...
- **Run file** (*implemented, T029*): `$(git rev-parse --git-common-dir)/taskrail/runs/<run>.json`, next to the claims,
  ...
  - per task: the lane handle, the group, `gate`, `escalated`, `parked` or `failed` with a reason,
    when `next` dispatched it, the allocated resources, and the governing paths the human approved
    with their blob IDs (T059). `escalated`, `parked` and `failed` are lane states git cannot
    derive: each keeps the task's claim, branch and worktree and frees its lane, and `parked` means
    the human has answered and the task waits for a lane (§12.7; T105);
...
**An escalated task frees its lane, and the answer puts it back in the queue** (*implemented,
T105*). It keeps its claim, branch and worktree, so nothing is lost and no other lane may take it,
but it holds no lane: `next` may dispatch another task in its place, and a run whose lanes are all
waiting for a human otherwise does nothing at all. When the human answers, the orchestrator records
`autopilot lane <ID> --run R --state parked --reason <the answer>` and runs `next --run R`, which
sends the task back — before any task that has never started, and only when a lane is free. The
orchestrator never decides on its own whether one is: the CLI computes what to dispatch (§12.1),
and an answer that arrives while every lane is busy leaves the task `parked` until one frees, which
`next` reports in `parked` and `status` as `holds_lane: false`.
...
- **A lane is in use** while its task is `running` (a claim, stale or not), `gate` or `dispatched`
  in a run. `escalated`, `parked`, `failed`, `done-branch`, `handed-off`, `done-merged`,
  `discarded-branch` and `discarded` use none. A task `escalated`, `parked` or `failed` keeps its
  claim, branch and worktree — so its task and dependents stay out — but no lane: it waits for a
  human, and a run whose lanes all waited would do nothing at all (*implemented, T105*; `failed`
  T030). A claim naming an open run whose task's row cannot be found in the checkout nor on its
...
- **A parked task comes back first.** `next --run R` dispatches the run's `parked` tasks — those
  whose escalation the human has answered — before any candidate, oldest answer first (the lane's
  `updated`, then the ID), subject to `max_lanes`, the group limits and the resources but **not**
  to the run's `count`, which already holds them. Each is recorded `running` with a fresh dispatch
  and fresh resource values, and its entry carries `restart: true` (*implemented, T105*).
...
- **The run's `count`** caps what a run starts: `next --run R` dispatches at most `count` minus the
  run's tasks that are `dispatched`, `running`, `gate`, `escalated`, `parked`, `failed`,
  `done-branch`, `handed-off` or `done-merged`; a named run's count is its number of named tasks,
  raised by `autopilot extend`. A `discarded` or `discarded-branch` task frees its place; an
  `escalated`, `parked` or `failed` one keeps it, since it waits for a human, and replacing it
  would start more work than was asked for. Freeing the lane therefore lets a run hold `max_lanes`
  working lanes **plus** the workspaces of every task waiting for a human; nothing caps those
  separately, and `status` reports each of them with `holds_lane: false` (*T105*).
